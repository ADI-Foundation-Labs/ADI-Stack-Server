...
> **STATUS (2026-04-21):** The feature as implemented in commit `e5ef976b` crashes the node at startup in K8s with `EADDRNOTAVAIL` — discv5's UDP bind is tied to the advertised IP, which isn't local to the pod. See `docs/superpowers/specs/2026-04-21-advertised-address-bind-fix-design.md` for the post-mortem and currently-blocked follow-up.
>
> **Correction to this plan, line 26:** the actual env key used by `smart_config` is `advertised_address` (no `network_` prefix). Setting `network_advertised_address` has no effect.
