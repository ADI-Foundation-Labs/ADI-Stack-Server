    ...
    async fn run(
        ...
        loop {
            ...
            let Some(record) = self.replays_for_sequencer.recv().await else {
                // The replay stream from the main node can drop (e.g. connection loss). Keep the
                // node alive and retry with backoff instead of tearing down the whole pipeline.
                tracing::warn!(
                    ?retry_delay,
                    current_block,
