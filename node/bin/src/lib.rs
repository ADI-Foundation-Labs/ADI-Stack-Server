...
async fn commit_proof_execute_block_numbers(
    ...
    // On External Nodes proofs are synced by a sidecar process with some delay, so batch lookups
    // get a grace period instead of failing on the first miss.
    let lookup = |batch_num: u64, context: &'static str| async move {
        if batch_num == 0 {
            return 0;
        }
        l1_watcher_util::retry_with_grace_period(
            ...
            &format!("{context} {batch_num} in proof storage"),
        )
        .await
        .unwrap_or_else(|e| panic!("failed during retry of {context} lookup: {e:?}"))
        .last_block_number()
    };
    (
        lookup(l1_state.last_committed_batch, "committed batch").await,
        lookup(l1_state.last_proved_batch, "proved batch").await,
        lookup(l1_state.last_executed_batch, "executed batch").await,
        lookup(
            l1_state.last_finalized_executed_batch,
            "finalized executed batch",
        )
        .await,
    )
}
