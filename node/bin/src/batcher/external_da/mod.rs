    ...
    pub async fn fetch_external_da_data(
        ...
        let publish_result = if let Some(url) = self.config.publish_url.as_deref() {
            ...
            let publish_response = self
                .post_with_retry(url, &publish_payload, request, &batch_key, ExternalDaOperation::Publish)
                .await
                .with_context(|| {
                    format!("failed to publish batch {} pubdata to external DA", request.batch_number)
                })?;
        ...
        let da_data = if let Some(data) = publish_result {
            ...
            let proof_response = self
                .post_with_retry(proof_url, &proof_payload, request, &batch_key, ExternalDaOperation::FetchProof)
                .await
                .with_context(|| {
                    format!("failed to fetch proof for external DA batch {}", request.batch_number)
                })?;
...
fn parse_da_data_from_response(
    ...
    match provider {
        ExternalDaProvider::Avail => {
            parse_response_with_wrappers::<AvailApiDaData>(&response)
...
fn parse_response_with_wrappers<T: serde::de::DeserializeOwned>(response: &Value) -> Option<T> {
    serde_json::from_value(response.clone())
        .ok()
        .or_else(|| response.get("data").and_then(|v| serde_json::from_value(v.clone()).ok()))
        .or_else(|| response.get("result").and_then(|v| serde_json::from_value(v.clone()).ok()))
}
