...
RUN cargo chef cook --bin zksync-os-server --release --features jemalloc-prof gcp --recipe-path recipe.json

# Build application
COPY . .
RUN cargo build --release --features jemalloc-prof gcp --bin zksync-os-server
