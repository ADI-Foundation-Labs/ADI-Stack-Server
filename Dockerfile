...
RUN cargo chef cook --bin zksync-os-server --release --features jemalloc-prof --recipe-path recipe.json

# Build application
COPY . .
RUN cargo build --release --features jemalloc-prof --bin zksync-os-server
