...
impl UpstreamGuard {
    ...
    pub(crate) fn current_upstream(&self) -> Option<PeerId> {
        *self
            .peer_id
            .lock()
...
mod tests {
    ...
    fn fresh_guard_has_no_upstream_and_first_acquire_wins() {
        ...
        assert_eq!(guard.current_upstream(), None);

        let permit = guard.try_acquire(peer_id(0x11));
        assert!(permit.is_some());
        assert_eq!(guard.current_upstream(), Some(peer_id(0x11)));
    }
    ...
    fn slot_is_reusable_after_drop_and_clear() {
        ...
        assert!(permit.is_some());
        assert_eq!(guard.current_upstream(), Some(peer_id(0x22)));
    }
}
